    ...
    <div className={cn(className, 'flex items-center justify-center gap-2')}>
      ...
      {currentPage !== 1 && (
        <Link href={`?page=${currentPage - 1}`}>
          <button className={cn('rounded-md bg-slate-200 p-2 hover:text-sky-600')}>
      ...
      {pageNumbers.map((pageNumber) => (
        <>
          ...
          {pageNumber === currentPage && (
            <button key={pageNumber} className={cn('px-3 py-1 rounded-md transition', {
              'bg-slate-200 hover:text-sky-600': pageNumber !== currentPage,
              'bg-blue-500 text-white cursor-default': pageNumber === currentPage,
            })}>
              {pageNumber}
            </button>
          )}
          {(pageNumber !== '...' && pageNumber !== currentPage) && (
            <Link href={`/?page=${pageNumber}`}>
              <button key={pageNumber} className={cn('px-3 py-1 rounded-md transition', {
                'bg-slate-200 hover:text-sky-600': pageNumber !== currentPage && pageNumber !== '...',
                'bg-blue-500 text-white cursor-default': pageNumber === currentPage,
